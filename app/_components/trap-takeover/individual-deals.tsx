  ...
  return (
    <section className="py-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {deals.map(
          ({ brand, dealType, category, description, options, details }) => (
            <div
              key={`${brand}-${dealType}-${category}-${description}`}
                  ...
                  <ul className="space-y-2">
                    {options.map(({ name, strain }) => (
                      <li
                        ...
                        <span
                          className={`${strainColors[strain]} rounded-full px-2 py-0.5 text-xs font-semibold text-white`}>
                          {strain.toUpperCase()}
                        </span>
